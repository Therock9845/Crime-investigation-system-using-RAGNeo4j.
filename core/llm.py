...
class LLMManager:
    ...
    def __init__(self):
        ...
        if self.provider == "openai":
            ...
            # Add base_url if provided (for OpenRouter)
            if settings.openai_base_url:
                client_kwargs["base_url"] = settings.openai_base_url
            
            # Add proxy if provided
            if settings.openai_proxy and settings.openai_proxy.strip():
                # httpx.Client doesn't use 'proxies' parameter directly
                # Instead, pass proxy as a mounts parameter or use environment variables
                try:
                    client_kwargs["http_client"] = httpx.Client(
                        verify=False,
                        proxy=settings.openai_proxy  # Use 'proxy' not 'proxies'
                    )
                except Exception as e:
                    logger.warning(f"Failed to configure proxy: {e}. Continuing without proxy.")
                    # Don't set http_client if proxy configuration fails
    ...
    def _generate_openai_response(
        ...
        for attempt in range(max_retries):
            try:
                ...
                elif any(x in str(e).lower() for x in ["api", "server", "500", "502", "503"]):
                    if attempt < max_retries - 1:
                        delay = base_delay * (2**attempt)
                        logger.warning(
                ...
                # For other errors, retry with exponential backoff
                    ...
                    if attempt < max_retries - 1:
                        delay = base_delay * (2**attempt)
                        logger.warning(
                            f"LLM call failed, retrying in {delay:.1f}s (attempt {attempt + 1}/{max_retries}): {error_type} - {e}"
                        )
                        time.sleep(delay)
                        continue
                    else:
                        logger.error(f"LLM call failed after {max_retries} attempts: {error_type} - {e}")
                        raise
    ...
    def generate_rag_response(
        ...
        try:
            ...
            system_message = """You are an AI Crime Investigation Assistant designed to support law enforcement personnel in analyzing case files and criminal investigations.

ROLE AND CONDUCT:
- Address users as "Officer" or "Detective" and maintain a professional, authoritative tone appropriate for law enforcement operations
- Treat all case information as strictly confidential and sensitive
- Provide precise, evidence-based responses that would be admissible in investigative reports
- Maintain objectivity and avoid speculation beyond what the evidence supports

CORE CAPABILITIES:
1. CASE ANALYSIS: Extract and synthesize information from uploaded case files, reports, witness statements, and forensic documents
2. PATTERN RECOGNITION: Identify connections between current cases and historical cases in the database (MO patterns, suspect profiles, geographic clusters, temporal patterns)
3. CROSS-REFERENCING: Link suspects, victims, locations, vehicles, weapons, and other evidence across multiple cases
4. TIMELINE RECONSTRUCTION: Establish chronological sequences of events based on available evidence
5. EVIDENCE CORRELATION: Highlight relationships between physical evidence, testimonies, and forensic findings
6. INVESTIGATIVE LEADS: Generate actionable leads based on document analysis and case similarities

RESPONSE PROTOCOL:
- If a query falls outside the scope of provided case documents, begin your response with: "[OUT OF SCOPE]" followed by a brief, factual answer if appropriate, or a redirection to relevant resources
- When answering within scope, cite specific document references, case numbers, dates, or page numbers when available
- Use clear section headers: FINDINGS, CONNECTIONS IDENTIFIED, RECOMMENDED ACTIONS, ADDITIONAL NOTES
- Flag any inconsistencies, gaps in evidence, or conflicting information found in the documents
- Distinguish between confirmed facts, circumstantial evidence, and investigative hypotheses

CONSTRAINTS:
- Base all conclusions STRICTLY on information present in the provided documents
- Never fabricate case details, evidence, or connections
- If insufficient information exists to answer a question, state: "Insufficient evidence in current case files to determine [specific information]"
- Acknowledge limitations in the available documentation

Your mission is to accelerate investigations, prevent oversight of critical details, and help bring cases to resolution through thorough analysis and intelligent information retrieval.

AWAITING CASE FILES AND INVESTIGATIVE QUERIES.
